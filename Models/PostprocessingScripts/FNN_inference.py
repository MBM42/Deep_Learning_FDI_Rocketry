...
Standalone script to perform inference using a trained Feedforward Neural Network.
...
# Add project root to import path, in order to provide the FNN architecture
project_root = os.path.abspath(os.path.join(current_dir, "..", ".."))
if project_root not in sys.path:
    sys.path.insert(0, project_root)
from FNN_main import FNN      # Model Definition


def get_config_values(config_filename: str = "Model_Config.txt"):
    """
    Read window_size, hidden_dim, drop_prob, features_h, labels_h from config.
    
    Params:
        config_filename (str): Name of the configuration file (Default: "Model_Config.txt")
    """

    window_size = hidden_dim = drop_prob = features_h = labels_h = None
    ...
    with open(cfg, "r") as f:
        for line in f:
            ...
            if all(v is not None for v in [window_size, hidden_dim, drop_prob, features_h, labels_h]):
                break
    return window_size, hidden_dim, drop_prob, features_h, labels_h
...
def main():
    ...
    window_size, hidden_dim, drop_prob, features_h, labels_h = get_config_values()

    # Load model
    input_dim = window_size * len(features_h)
    model = FNN(input_dim, hidden_dim, len(labels_h), drop_prob).to(device)
    ...
    for filepath in tqdm(npy_files, desc="Evaluating Inference Metrics", unit="file"):
        ...
        # Stack and flatten
        windows = np.stack([
            features[i : i + window_size].reshape(-1)
